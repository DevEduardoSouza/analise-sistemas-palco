...
As categorias seguem o modelo de Documento de Visão da disciplina: Usabilidade, Segurança, Performance e Portabilidade. Foram acrescentadas duas categorias complementares, Confiabilidade e Conformidade legal, por serem determinantes num sistema de bilheteria.
...
| RNF05 | Painel do produtor | O produtor deve conseguir publicar um evento simples, uma sessão e um lote, em menos de dez minutos sem treinamento formal. |
...
| RNF10 | Painel administrativo | O acesso aos perfis Administrador e Financeiro deve exigir segundo fator de autenticação. |
...
| RNF13 | Bilheteria | O sistema deve suportar mil compras simultâneas na abertura de vendas de um evento de grande porte, sem venda acima da capacidade do setor. |
| RNF14 | Aplicativo de portaria | A validação de um ingresso deve responder em até dois segundos, e a portaria deve sustentar a entrada de mil e duzentas pessoas por hora por portão. |
...
| RNF19 | Dados | O sistema deve permitir exportação completa dos dados do produtor em formato aberto, CSV e JSON, garantindo a portabilidade prevista na legislação. |
...
| RNF21 | Aplicativo de portaria | A portaria deve continuar operando por no mínimo seis horas sem conexão de rede, sincronizando ao restabelecer o acesso. |
...
| RNF26 | Auditoria | Os registros de auditoria e os documentos fiscais devem ser retidos por cinco anos. |
